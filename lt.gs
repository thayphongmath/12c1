function doGet(e) {
  ...
  try {
    var template = HtmlService.createTemplateFromFile('lt');
    return template.evaluate()
        .setTitle('Thi Đua Lớp Trưởng (LT)')
        .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
  } catch (error) {
    Logger.log('Error loading lt.html: ' + error.message);
    return ContentService
        .createTextOutput('Lỗi: Không tìm thấy tệp HTML lt. Vui lòng kiểm tra Project Explorer.')
...
function doPost(e) {
  try {
    ...
    var sheet = SpreadsheetApp.openById('1GS3QKu5Bbeoef0rQhkitX_dawnwqlRPdeUaCNKV8cJk').getSheetByName('LT');

    if (data.password !== 'loptruong') {
