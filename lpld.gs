function doGet(e) {
  ...
  try {
    var template = HtmlService.createTemplateFromFile('lpld');
    return template.evaluate()
        .setTitle('Thi Đua Lớp Phó Lao Động (LPLD)')
        .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
  } catch (error) {
    Logger.log('Error loading lpld.html: ' + error.message);
    return ContentService
        .createTextOutput('Lỗi: Không tìm thấy tệp HTML lpld. Vui lòng kiểm tra Project Explorer.')
...
function doPost(e) {
  try {
    ...
    var sheet = SpreadsheetApp.openById('1GS3QKu5Bbeoef0rQhkitX_dawnwqlRPdeUaCNKV8cJk').getSheetByName('LPLD');

    if (data.password !== 'loppholaodong') {
