function doGet(e) {
  ...
  try {
    var template = HtmlService.createTemplateFromFile('lpht');
    return template.evaluate()
        .setTitle('Thi Đua Lớp Phó Học Tập (LPHT)')
        .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
  } catch (error) {
    Logger.log('Error loading lpht.html: ' + error.message);
    return ContentService
        .createTextOutput('Lỗi: Không tìm thấy tệp HTML lpht. Vui lòng kiểm tra Project Explorer.')
...
function doPost(e) {
  try {
    ...
    var sheet = SpreadsheetApp.openById('1GS3QKu5Bbeoef0rQhkitX_dawnwqlRPdeUaCNKV8cJk').getSheetByName('LPHT');

    if (data.password !== 'lopphohoctap') {
